This is the Security Policy for the Perl distribution App-ipgrep (ipgrep).
...
Any security vulnerabilities in ipgrep are covered
by this policy.
...
by this policy unless they affect ipgrep, or
ipgrep can be used to exploit vulnerabilities in
them.

Security vulnerabilities in downstream software (any software that
uses ipgrep, or plugins to it that are not included
with the ipgrep distribution) are not covered by
this policy.

## Which Versions of this Software are Supported?

The maintainer(s) will only commit to releasing security fixes for the
latest version of ipgrep.

Note that the ipgrep project only supports major
versions of Perl released in the past ten (10) years, even though
ipgrep will run on older versions of Perl.  If a
security fix requires us to increase the minimum version of Perl that
is supported, then we may do that.

# Installation and Usage Issues

Please see the application documentation for more information.
